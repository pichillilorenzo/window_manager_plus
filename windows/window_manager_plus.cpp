...
namespace window_manager_plus {
	...
	WindowManagerPlusPluginWindowCreatedCallback g_window_created_callback = nullptr;
	...
	int64_t WindowManagerPlus::createWindow(const std::vector<std::string>& args) {
		if (g_window_created_callback) {
			...
			g_window_created_callback(windowId, std::move(dst));
			return windowId;
		}
		return -1;
	}
	...
	void WindowManagerPlus::ForceChildRefresh() {
		...
		GetWindowRect(hWnd, &rect);
		SetWindowPos(
			hWnd, nullptr, rect.left, rect.top, rect.right - rect.left + 1, rect.bottom - rect.top, SWP_NOZORDER | SWP_NOOWNERZORDER | SWP_NOMOVE | SWP_FRAMECHANGED);
		SetWindowPos(hWnd, nullptr, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, SWP_NOZORDER | SWP_NOOWNERZORDER | SWP_NOMOVE | SWP_FRAMECHANGED);
	}
	...
	void WindowManagerPlus::WaitUntilReadyToShow() {
		::CoCreateInstance(CLSID_TaskbarList, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&taskbar_));
	}
	...
	void PASCAL WindowManagerPlus::AppBarQuerySetPos(HWND hwnd, UINT uEdge, LPRECT lprc, PAPPBARDATA pabd) {
		int iHeight = 0;
		int iWidth = 0;
	...
	void WindowManagerPlus::SetFullScreen(const flutter::EncodableMap& args) {
		...
		if (isFullScreen) {  // Set to fullscreen
			...
			if (!is_frameless_) {
				...
				::SetWindowPos(mainWindow, HWND_TOP, monitor.rcMonitor.left, monitor.rcMonitor.top, monitor.rcMonitor.right - monitor.rcMonitor.left, monitor.rcMonitor.bottom - monitor.rcMonitor.top, SWP_NOOWNERZORDER | SWP_FRAMECHANGED);
			}
		}
			...
			if (::IsZoomed(mainWindow)) {
				...
				auto flutter_view = ::FindWindowEx(mainWindow, nullptr, kFlutterViewWindowClassName, nullptr);
				...
				::SetWindowPos(mainWindow, nullptr, g_frame_before_fullscreen.left, g_frame_before_fullscreen.top, g_frame_before_fullscreen.right - g_frame_before_fullscreen.left, g_frame_before_fullscreen.bottom - g_frame_before_fullscreen.top, SWP_NOACTIVATE | SWP_NOZORDER);
			}
		}
	}

	void WindowManagerPlus::SetAspectRatio(const flutter::EncodableMap& args) {
		aspect_ratio_ = std::get<double>(args.at(flutter::EncodableValue("aspectRatio")));
	}
	...
	void WindowManagerPlus::SetMaximizable(const flutter::EncodableMap& args) {
		HWND hWnd = GetMainWindow();
		bool isMaximizable = std::get<bool>(args.at(flutter::EncodableValue("isMaximizable")));
		DWORD gwlStyle = GetWindowLong(hWnd, GWL_STYLE);
	...
	void WindowManagerPlus::SetAlwaysOnBottom(const flutter::EncodableMap& args) {
		is_always_on_bottom_ = std::get<bool>(args.at(flutter::EncodableValue("isAlwaysOnBottom")));
	...
	int WindowManagerPlus::GetTitleBarHeight() {
		...
		int height = ptinfo->rcTitleBar.bottom == 0 ? 0 : ptinfo->rcTitleBar.bottom - ptinfo->rcTitleBar.top;
		free(ptinfo);
	...
	void WindowManagerPlus::SetSkipTaskbar(const flutter::EncodableMap& args) {
		is_skip_taskbar_ = std::get<bool>(args.at(flutter::EncodableValue("isSkipTaskbar")));
	...
	void WindowManagerPlus::SetProgressBar(const flutter::EncodableMap& args) {
		double progress = std::get<double>(args.at(flutter::EncodableValue("progress")));

		HWND hWnd = GetMainWindow();
		taskbar_->SetProgressState(hWnd, TBPF_INDETERMINATE);
		taskbar_->SetProgressValue(hWnd, static_cast<int32_t>(progress * 100), static_cast<int32_t>(100));

		if (progress < 0) {
			taskbar_->SetProgressState(hWnd, TBPF_NOPROGRESS);
			taskbar_->SetProgressValue(hWnd, static_cast<int32_t>(0), static_cast<int32_t>(0));
		}
		else if (progress > 1) {
			taskbar_->SetProgressState(hWnd, TBPF_INDETERMINATE);
			taskbar_->SetProgressValue(hWnd, static_cast<int32_t>(100), static_cast<int32_t>(100));
		}
		else {
			taskbar_->SetProgressState(hWnd, TBPF_INDETERMINATE);
			taskbar_->SetProgressValue(hWnd, static_cast<int32_t>(progress * 100), static_cast<int32_t>(100));
	...
	void WindowManagerPlus::SetIcon(const flutter::EncodableMap& args) {
		...
		HICON hIconSmall = (HICON)(LoadImage(NULL, (LPCWSTR)(converter.from_bytes(iconPath).c_str()), IMAGE_ICON, 16, 16, LR_LOADFROMFILE));

		HICON hIconLarge = (HICON)(LoadImage(NULL, (LPCWSTR)(converter.from_bytes(iconPath).c_str()), IMAGE_ICON, 32, 32, LR_LOADFROMFILE));
	...
	void WindowManagerPlus::SetBrightness(const flutter::EncodableMap& args) {
		...
		if (result == ERROR_SUCCESS) {
			std::string brightness = std::get<std::string>(args.at(flutter::EncodableValue("brightness")));
			HWND hWnd = GetMainWindow();
	...
	void WindowManagerPlus::PopUpWindowMenu(const flutter::EncodableMap& args) {
		...
		int cmd = TrackPopupMenu(hMenu, TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_RETURNCMD, static_cast<int>(x), static_cast<int>(y), 0, hWnd, NULL);
...
void WindowManagerPlusPluginSetWindowCreatedCallback(WindowManagerPlusPluginWindowCreatedCallback callback) {
