...
#pragma warning(pop)
	...
	private:
		...
		std::optional<LRESULT> WindowManagerPlusPlugin::HandleWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
		// Called when a method is called on this plugin's channel from Dart.
		void HandleMethodCall(const flutter::MethodCall<flutter::EncodableValue>& method_call, std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result);

		static void HandleStaticMethodCall(const flutter::MethodCall<flutter::EncodableValue>& method_call, std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result);
	...
	// static
	void WindowManagerPlusPlugin::RegisterWithRegistrar(flutter::PluginRegistrarWindows* registrar) {
		auto plugin = std::make_unique<WindowManagerPlusPlugin>(registrar);

		registrar->AddPlugin(std::move(plugin));
	}

	WindowManagerPlusPlugin::WindowManagerPlusPlugin(flutter::PluginRegistrarWindows* registrar) : registrar(registrar) {
		window_manager = std::make_shared<WindowManagerPlus>();
		window_manager->static_channel = std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(registrar->messenger(), "window_manager_plus_static", &flutter::StandardMethodCodec::GetInstance());
		window_manager->static_channel->SetMethodCallHandler(
			[](const auto& call, auto result) {
				HandleStaticMethodCall(call, std::move(result));
			});
		window_manager->channel = std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(registrar->messenger(), "window_manager_plus", &flutter::StandardMethodCodec::GetInstance());
		window_manager->channel->SetMethodCallHandler([this](const auto& call, auto result) {
	...
	WindowManagerPlusPlugin::~WindowManagerPlusPlugin() {
		...
		WindowManagerPlus::windowManagers_.erase(window_manager->id);
	}
	...
	void WindowManagerPlusPlugin::_EmitEvent(std::string eventName) {
		...
		window_manager->channel->InvokeMethod("onEvent", std::make_unique<flutter::EncodableValue>(args));
			...
			if (wManagerPair.second->channel) {
				wManagerPair.second->channel->InvokeMethod(
					...
					std::make_unique<flutter::EncodableValue>(flutter::EncodableMap{
						{flutter::EncodableValue("eventName"), flutter::EncodableValue(eventName)},
						{flutter::EncodableValue("windowId"), flutter::EncodableValue(window_manager->id)}
						}));
			}
		}
	}
	...
	std::optional<LRESULT> WindowManagerPlusPlugin::HandleWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
		...
		if (wParam && message == WM_NCCALCSIZE) {
			...
			if (window_manager->aspect_ratio_ > 0) {
				...
				bool is_resizing_horizontally = wParam == WMSZ_LEFT || wParam == WMSZ_RIGHT || wParam == WMSZ_TOPLEFT || wParam == WMSZ_BOTTOMLEFT;
	...
	void WindowManagerPlusPlugin::HandleStaticMethodCall(
		...
		const flutter::EncodableMap& args = method_call.arguments()->IsNull() ? flutter::EncodableMap() : std::get<flutter::EncodableMap>(*method_call.arguments());
		/*auto windowId =
		...
		if (method_name.compare("createWindow") == 0) {
			auto encodedArgs = args.at(flutter::EncodableValue("args")).IsNull() ? flutter::EncodableList() : std::get<flutter::EncodableList>(args.at(flutter::EncodableValue("args")));
			...
			result->Success(newWindowId >= 0 ? flutter::EncodableValue(newWindowId) : flutter::EncodableValue());
		}
	...
	void WindowManagerPlusPlugin::HandleMethodCall(
		...
		const flutter::EncodableMap& args = method_call.arguments()->IsNull() ? flutter::EncodableMap() : std::get<flutter::EncodableMap>(*method_call.arguments());
		auto windowId = args.find(flutter::EncodableValue("windowId")) != args.end() ? std::get<int>(args.at(flutter::EncodableValue("windowId"))) : -1;
		...
		if (method_name.compare("ensureInitialized") == 0) {
			if (windowId >= 0) {
				...
				window_manager->channel =
					std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(registrar->messenger(), "window_manager_plus_" + std::to_string(windowId), &flutter::StandardMethodCodec::GetInstance());
				window_manager->channel->SetMethodCallHandler([this](const auto& call, auto result) {
					HandleMethodCall(call, std::move(result));
					});
			...
			auto targetWindowId = std::get<int>(args.at(flutter::EncodableValue("targetWindowId")));
			if (WindowManagerPlus::windowManagers_.find(targetWindowId) != WindowManagerPlus::windowManagers_.end()) {
				auto result_ = std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>>(std::move(result));
				WindowManagerPlus::windowManagers_[targetWindowId]->channel->InvokeMethod("onEvent", std::make_unique<flutter::EncodableValue>(args.at(flutter::EncodableValue("args"))),
					std::make_unique<
					flutter::MethodResultFunctions<flutter::EncodableValue>>([result_](const flutter::EncodableValue* val) {
						// Success
						result_->Success(*val);
						},
						[result_](const std::string& error_code, const std::string& error_message, const flutter::EncodableValue* error_details) {
							// Error
...
void WindowManagerPlusPluginRegisterWithRegistrar(FlutterDesktopPluginRegistrarRef registrar) {
	window_manager_plus::WindowManagerPlusPlugin::RegisterWithRegistrar(flutter::PluginRegistrarManager::GetInstance()->GetRegistrar<flutter::PluginRegistrarWindows>(registrar));
}
